...
Сценарий: Вася не может добавлять запись в справочник лечений
	...
	То мне должен быть ответ "У вас нет прав доступа на добавление записи!"
```
...
Функция: поддержка многих справочников
	...
	Предыстория:
		...
		И у пользователя "Вася" есть право на запись в справочник "Лекарство"
...
- Создавать объекты сущностей или настраивать БД
```
Дано нет пользователей в базе
Дано база данных пустая
```
- Вход пользователя в систему (исключение к правилу "никаких взаимодействий в шаге Дано")
```
Дано я вошел в систему как "Вася"
```
